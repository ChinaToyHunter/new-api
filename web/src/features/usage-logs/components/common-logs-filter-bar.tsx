...
import { getRouteGroups } from '@/features/users/api'
...
export function CommonLogsFilterBar<TData>(
  ...
  const { data: adminGroups } = useQuery({
    ...
    queryFn: async () => requireServerSuccess(await getRouteGroups()),
    enabled: isAdmin,
  })
  const { data: userGroups } = useQuery({
    queryKey: ['user-route-groups'],
    queryFn: async () => requireServerSuccess(await getUserRouteGroups()),
    enabled: !isAdmin,
  })
  ...
  const handleApply = useCallback(
    (nextFilters: CommonLogFilters = filters) => {
      const modelName = nextFilters.model?.trim() ?? ''
      const modelNameMode =
        nextFilters.modelNameMode ?? DEFAULT_MODEL_NAME_MODE
      ...
      const filterParams = buildSearchParams(
        { ...nextFilters, model: modelName || undefined },
        'common'
      )
      navigate({
