...
export function transformChannelToFormDefaults(
  ...
  return {
    ...
    user_groups: parseGroups(channel.user_groups || ''),
...
export function transformFormDataToCreatePayload(formData: ChannelFormValues): {
  ...
  const channel: Partial<Channel> = {
    ...
    user_groups: formatGroups(formData.user_groups || []),
...
export function transformFormDataToUpdatePayload(
  ...
  // Clearing user_groups must reach the backend as an empty string, not null.
  payload.user_groups = formatGroups(formData.user_groups || [])
