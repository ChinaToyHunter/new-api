...
func GenerateOAuthCode(c *gin.Context) {
	...
	// hidden mode: never store invitation codes into the OAuth auth flow
	if common.RegistrationInviteMode == common.RegistrationInviteModeHidden {
		request.Aff = ""
	}
...
func findOrCreateOAuthUser(c *gin.Context, provider oauth.Provider, oauthUser *oauth.OAuthUser, token *oauth.OAuthToken, affiliateCode string) (*model.User, *service.LegacyGitHubMigration, error) {
	...
	// Handle affiliate code (server-side invite mode is authoritative)
	inviterId, err := resolveConfiguredRegistrationInviter(affiliateCode)
	if err != nil {
		return nil, nil, err
	}
