...
`
	...
	for index, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			user := model.User{Username: fmt.Sprintf("task_user_%d", index), AffCode: fmt.Sprintf("task_aff_%d", index), Quota: int64(initial)}
			...
			assert.Equal(t, int64(initial-want), updated.Quota)
			...
			require.NoError(t, db.First(&updated, user.Id).Error)
			assert.Equal(t, int64(initial-want), updated.Quota, "terminal settlement is idempotent")
		})
	}
}
