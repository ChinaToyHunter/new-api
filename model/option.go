...
func InitOptionMap() {
	...
	common.OptionMap["SystemUpdateTestModeEnabled"] = strconv.FormatBool(operation_setting.SystemUpdateTestModeEnabled)
...
func loadOptionsFromDatabase() {
	...
	// Passkey domain options configure the WebAuthn relying party as a set and
	// are not generic option-map entries, so they cannot go through the loop.
...
func updateOptionMap(key string, value string) (err error) {
	...
	if strings.HasSuffix(key, "Enabled") || key == "DefaultCollapseSidebar" || key == "DefaultUseAutoGroup" || key == "SMTPForceAuthLogin" || key == "SMTPInsecureSkipVerify" {
		...
		case "SystemUpdateTestModeEnabled":
			operation_setting.SystemUpdateTestModeEnabled = boolValue
